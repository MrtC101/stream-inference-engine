...
Este documento describe el estado actual, supuestos y limitaciones del sistema en contexto de **producción MVP**. No define una arquitectura final ni compromisos de SLA. Su objetivo es dejar explícito qué comportamientos están garantizados, cuáles no, y qué aspectos requieren ingeniería adicional.
...
Lo único que no está cerrado (y quedó intencionalmente marcado como tal en el documento) es:

Criterios formales de fatal vs recoverable failure.

Política concreta de retry / backoff / abort.

Definición mínima de métricas obligatorias para operación.

Estrategia futura de orquestación / HA (solo como nota, no diseño).

Política de configuración y seguridad (fuera de scope pero mencionada).